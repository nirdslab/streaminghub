...
from ..fixation_detection.typing import FixationEvent, SaccadeEvent
...
class WhiteNoiseSimulator(datamux.PipeTask):
    ...
    def synthesize_fixation(self, f: FixationEvent) -> list[dict]:
    ...
    def __call__(self, *args, **kwargs) -> None:
        ...
        if item is None:
            return
        if isinstance(item, SaccadeEvent):
            data = self.synthesize_saccade(item)
        elif isinstance(item, FixationEvent):
