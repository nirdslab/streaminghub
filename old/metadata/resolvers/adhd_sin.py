...
def resolve(spec: dfds.DataSetSpec, **kwargs) -> Iterator[Tuple[DICT, str]]:
  ...
  logger.info(f"requested: "
              ...
              f"[Diagnosis]:{','.join(diagnosis):.40}{'...' * bool(','.join(diagnosis)[40:])}, "
              f"[Question]:{','.join(question):.40}{'...' * bool(','.join(question)[40:])}, "
              f"[Noise]:{','.join(noise):.40}{'...' * bool(','.join(noise)[40:])}")
...
def stream(spec: dfds.DataSetSpec, **kwargs) -> Iterator[Tuple[DICT_GENERATOR, DICT]]:
