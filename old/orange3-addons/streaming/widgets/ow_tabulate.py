...
class TabulateWidget(widget.OWWidget):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self.streams = []  # type: List[pylsl.StreamInlet]
    # ui
  ...
  class Inputs:
    ...
    Inputs
    """
    streams = Input("Streams", pylsl.StreamInlet)
  ...
  @Inputs.streams
  def set_streams(self, streams: List[pylsl.StreamInlet]):
...
class Task(QObject):
  streams = ...  # type: List[pylsl.StreamInlet]
  ...
  def __init__(self,
               streams: List[pylsl.StreamInlet],
  ...
  def init_ch_map(self):
    ...
    for inlet in self.streams:
      info = inlet.info()
      ch_labels = []
      ptr: pylsl.XMLElement = info.desc()
  ...
  def _run(self):
    ...
    if updated:
      ...
      table = Table.from_numpy(Domain([TimeVariable(self.df.index.name), *map(ContinuousVariable, self.df.columns)]),
                               table_data)
      # trigger callback
      self.callback(table)
